 ...
 */
float AP_Mount_Alexmos::compensate_mount_imu()
{

    uint8_t mntCal_mode = _state._ahrs_helper;
    gcs().send_text(MAV_SEVERITY_INFO, "ahrs helper is :%u",(unsigned)mntCal_mode);
    ...
    f2m.b.y =  sin_phi*sin_tht*sin_ksi + cos_tht*cos_ksi;    if (!mode) {
