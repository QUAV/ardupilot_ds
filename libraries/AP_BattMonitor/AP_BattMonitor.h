...
class AP_BattMOnitor_Gov;

class AP_BattMonitor
{
    friend class AP_BattMonitor_Backend;
    friend class AP_BattMonitor_Gov;
...
public:
    ...
    /// generator_amps - returns current generated by motor
    float generator_amps(uint8_t instance) const;
    float generator_amps() const { return generator_amps(AP_BATT_PRIMARY_INSTANCE); }

    /// rotor_amps - returns 
    float rotor_amps(uint8_t instance) const;
    float rotor_amps() const { return rotor_amps(AP_BATT_PRIMARY_INSTANCE); }
    ...
    float get_resistance(uint8_t instance) const { return state[0].resistance; }
