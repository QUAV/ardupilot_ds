...
AP_BattMonitor_Gov::AP_BattMonitor_Gov(AP_BattMonitor &mon, AP_BattMonitor::BattMonitor_State &mon_state, AP_BattMonitor_Params &params) :
...
void AP_BattMonitor_Gov::init(const AP_SerialManager& serial_manager)
{
    if ((_port = serial_manager.find_serial(AP_SerialManager::SerialProtocol_Gov, 0))) {        
        _initialised = true;
        _step = 0;
        _last_reading_ms = 0;
    }
...
void AP_BattMonitor_Gov::parse_body()
{
    ...
    _state.gas_percent = (uint8_t)(_buffer.Batt_data.pwm_throttle * 0.1f);
}


// Read inconmming message

bool AP_BattMonitor_Gov::read_incoming()
{
    uint8_t data;
    int16_t numc;
